...
mod linear_algebra_tests {
    ...
    #[test]
    fn matrix_creation() {
        let matrix: Matrix = Matrix::new(2, 2, vec![vec![1.0, 1.5], vec![2.0, 2.5]]);
    ...
    fn matrix_multiplication_should_fail_when_matrix_a_rows_does_not_match_matrix_b_cols() {
        let a = vec![vec![1.0, 10.0], vec![4.0, 4.0], vec![9.0, 7.0]];
        let b = vec![vec![3.0, 5.0], vec![5.0, 7.0]];
    ...
    fn matrix_2x2_multiplication() {
        let a = vec![vec![1.0, 10.0], vec![4.0, 9.0]];
        let b = vec![vec![3.0, 5.0], vec![5.0, 7.0]];
    ...
    fn matrix_2x1_adition() {
        let a = vec![vec![1.0], vec![4.0]];
        let b = vec![vec![3.0], vec![5.0]];
        let matrix_a: Matrix = Matrix::new(2, 1, a);
        let matrix_b: Matrix = Matrix::new(2, 1, b);
        let added_matrix: Matrix = matrix_a.add(&matrix_b);
    ...
    #[test]
    fn matrix_2x1_hadamard_product() {
        let a = vec![vec![1.0], vec![4.0]];
        let b = vec![vec![3.0], vec![5.0]];
        let matrix_a: Matrix = Matrix::new(2, 1, a);
        let matrix_b: Matrix = Matrix::new(2, 1, b);
    ...
    #[test]
    fn transpose_matrix() {
        let a = vec![vec![1.0, 10.0, 5.0], vec![4.0, 9.0, 3.0]];
        let matrix_a: Matrix = Matrix::new(2, 3, a);
    ...
    fn identity() {
        ...
        assert_eq!(matrix_a.data[0][0], 1.0);
        assert_eq!(matrix_a.data[0][1], 0.0);
        assert_eq!(matrix_a.data[0][2], 0.0);

        assert_eq!(matrix_a.data[1][0], 0.0);
        assert_eq!(matrix_a.data[1][1], 1.0);
        assert_eq!(matrix_a.data[1][2], 0.0);

        assert_eq!(matrix_a.data[2][0], 0.0);
        assert_eq!(matrix_a.data[2][1], 0.0);
        assert_eq!(matrix_a.data[2][2], 1.0);
    }
